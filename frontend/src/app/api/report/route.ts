...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { workflowCounts, totalSubplates } = await getCached(`report_workflow_summary`, 30, async () => {
      const [scansRes, subplateCountRes] = await Promise.all([
        supabaseAdmin.from("scan").select("worktype"),
        supabaseAdmin.from("subplate").select("id", { count: "exact", head: true }).is("deleted_at", null),
      ]);
      ...
      for (const s of scans) {
        ...
        if (wt.includes("pulp")) counts.pulpMould++;
        else if (wt.includes("tf")) counts.tf++;
        else if (wt.includes("rework") || wt.includes("r.e.")) counts.rework++;
        else if (wt.includes("machine")) counts.machinePart++;
        else if (wt.includes("accessories")) counts.accessories++;
        else if (wt.includes("sample")) counts.sample++;
        else counts.other++;
      }

      return {
        workflowCounts: counts,
        totalSubplates: subplateCountRes.count || 0,
      };
    });
